...
use iced::{
    alignment, widget::{column, container, horizontal_rule, row, scrollable, text, button, Space},
...
pub fn view(state: &JobTracker) -> Element<'_, Message, Theme> {
    ...
    // Get filtered jobs based on search query and filter status
    let jobs_to_display = if state.search_query.is_empty() && state.filter_status.is_none() {
        // If no filters active, use all jobs
        state.jobs.iter().enumerate().collect::<Vec<_>>()
    } else {
        // Otherwise use filtered jobs
        state.filtered_jobs().into_iter().enumerate().collect::<Vec<_>>()
    };

    // Create job rows with enhanced styling (with inline edit form)
    let job_rows = jobs_to_display.iter().fold(
        column![].spacing(12),
        |col, &(index, job)| {
    ...
    let applied_count = state.jobs.iter().filter(|j| j.status.to_string() == "Applied").count();
    let rejected_count = state.jobs.iter().filter(|j| j.status.to_string() == "Rejected").count();
    let offer_count = state.jobs.iter().filter(|j| j.status.to_string() == "Offer").count();
    ...
    let stats_row = row![
        ...
        text(format!("Showing {} of {} applications", jobs_to_display.len(), state.jobs.len()))
            .size(12)
            .style(|_| text::Style { color: Some(kraken_secondary_text()) }),
    ]
        .spacing(15)
